...
export default async function SearchPage(props: {
  ...
  const t = await getTranslations()
  return (
    <div className="enhanced-padding">
      <div className='my-4 bg-transparent-card md:border-b flex-between flex-col md:flex-row enhanced-padding rounded-lg'>
      ...
      <div className='bg-transparent-card grid md:grid-cols-5 md:gap-6'>
        <CollapsibleOnMobile title={t('Search.Filters')} className="search-filter-section">
        ...
        <div className='md:col-span-4 search-result-section'>
          <div>
            {t('Search.Check each product page for other buying options')}
          </div>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 my-4">
            {data && data.products && data.products.length > 0 ? (
              data.products.map((product: IProduct) => (
                <ProductCard key={product._id} product={product} />
              ))
            ) : (
              <div>{t('Search.No results found')}</div>
            )}
          </div>
          {data.totalPages > 1 && (
