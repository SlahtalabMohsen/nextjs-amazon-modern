...
const ProductCard = ({
    ...
    try {
      ...
      if (locale !== 'en-US') {
        ...
        if (name.includes("Fossil Men's Machine")) {
          return t('ProductNames.Fossil Men\'s Machine Stainless Steel Watch', { fallback: name })
        }
        if (name.includes("Mens Wearbreeze")) {
          return t('ProductNames.Mens Wearbreeze Shoes, Urban - Ultra Lightweight', { fallback: name })
        }
        if (name.includes("DLWKIPV Mens")) {
          return t('ProductNames.DLWKIPV Mens Running Shoes', { fallback: name })
        }
        if (name.includes("Casio Classic")) {
          return t('ProductNames.Casio Men\'s Heavy Duty Design Watch', { fallback: name })
        }
        
        // Try general category names
        for (const category of ["Jeans", "Shoes", "T-Shirts", "Wrist Watches"]) {
  ...
  const ProductDetails = () => (
    <div className='flex-1 space-y-3 p-2 animate-fade-in' style={{ animationDelay: '0.2s' }}>
      <p className='font-bold text-primary neumorphic-text dark:neumorphic-text-dark'>{product.brand}</p>
      <Link
